...
const MOCK_REQUESTS: LibraryNOC[] = [
  {
    id: 'LB001',
    ...
    semester: 'Semester 6',
    requestType: 'Library Clearance NOC',
  ...
  },
  {
    ...
    borrowedBooks: 0,
    outstandingFines: 0,
    remarks: 'All books returned, no dues'
  },
  {
    id: 'LB005',
    ...
    remarks: 'Books not returned, fines pending'
  }
];

const LibraryDept: React.FC = () => {
  const [requests, setRequests] = useState<LibraryNOC[]>(MOCK_REQUESTS);
  const [currentPage, setCurrentPage] = useState(1);
  const [searchQuery, setSearchQuery] = useState('');
  const [filterStatus, setFilterStatus] = useState<string>('All');
  const [selectedRequest, setSelectedRequest] = useState<LibraryNOC | null>(null);
  ...
  const handleAction = (request: LibraryNOC, action: 'approve' | 'reject') => {
    ...
    const updatedRequests = requests.map(req => {
      if (req.id === selectedRequest.id) {
        return {
          ...
          remarks: remarks || (actionType === 'approve' ? 'Library NOC Approved' : 'Library NOC Rejected')
        } as LibraryNOC;
      }
      return req;
    });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <header>
        ...
        <h1 className="text-4xl font-semibold text-stone-900 leading-tight">Library Department</h1>
        <p className="text-sm text-stone-500 mt-2">
          Review and approve library clearance NOC requests
        </p>
      </header>
      ...
      <div className="grid grid-cols-4 gap-4">
        ...
        <div className="bg-gradient-to-br from-purple-50 to-purple-100 border border-purple-200 rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div>
              <p className="text-xs font-semibold text-purple-600 uppercase tracking-wider mb-1">Total</p>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-stone-50/50 border-b border-stone-200">
                ...
                <th className="px-6 py-4 text-[10px] font-semibold text-stone-400 uppercase tracking-widest">Borrowed Books</th>
                <th className="px-6 py-4 text-[10px] font-semibold text-stone-400 uppercase tracking-widest">Outstanding Fines</th>
            ...
            <tbody className="divide-y divide-stone-100">
              {currentRecords.map((record) => (
                <tr key={record.id} className="hover:bg-stone-50/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`text-sm font-bold ${record.borrowedBooks > 0 ? 'text-red-600' : 'text-green-600'}`}>
                      {record.borrowedBooks}
                    </span>
                  </td>
                  <td className="px-6 py-4">
                    <span className={`text-sm font-bold ${record.outstandingFines > 0 ? 'text-red-600' : 'text-green-600'}`}>
                      ₹{record.outstandingFines}
                    </span>
                  </td>
                  <td className="px-6 py-4">
          ...
          <div className="bg-white rounded-3xl shadow-2xl max-w-md w-full p-8 animate-in fade-in zoom-in duration-300">
            <div className="text-center mb-6">
              ...
              <h3 className="text-2xl font-bold text-stone-900 mb-2">
                {actionType === 'approve' ? 'Approve Library NOC' : 'Reject Library NOC'}
              </h3>
