...
const MOCK_REQUESTS: AdmissionNOC[] = [
  {
    id: 'AD001',
  ...
  },
  {
    ...
    course: 'MBA Finance',
    semester: 'Semester 4',
    requestType: 'Migration Certificate',
  ...
  },
  {
    ...
    status: 'Rejected',
    documentStatus: 'Incomplete',
    remarks: 'Required documents not submitted'
  }
];

const AdmissionDept: React.FC = () => {
  const [requests, setRequests] = useState<AdmissionNOC[]>(MOCK_REQUESTS);
  const [currentPage, setCurrentPage] = useState(1);
  const [searchQuery, setSearchQuery] = useState('');
  const [filterStatus, setFilterStatus] = useState<string>('All');
  const [selectedRequest, setSelectedRequest] = useState<AdmissionNOC | null>(null);
  ...
  const handleAction = (request: AdmissionNOC, action: 'approve' | 'reject') => {
    ...
    const updatedRequests = requests.map(req => {
      if (req.id === selectedRequest.id) {
        return {
          ...
          remarks: remarks || (actionType === 'approve' ? 'Request Approved' : 'Request Rejected')
        } as AdmissionNOC;
      }
      return req;
    });
  ...
  return (
    <div className="space-y-6 animate-in fade-in duration-500">
      <header>
        ...
        <h1 className="text-4xl font-semibold text-stone-900 leading-tight">Admission Department</h1>
        <p className="text-sm text-stone-500 mt-2">
          Review and approve certificate and NOC requests
        </p>
      </header>
        ...
        <div className="bg-gradient-to-br from-indigo-50 to-indigo-100 border border-indigo-200 rounded-2xl p-6">
          <div className="flex items-center justify-between">
            <div>
              ...
              <p className="text-3xl font-bold text-indigo-700">{requests.length}</p>
            </div>
          ...
          <table className="w-full text-left border-collapse">
            <thead>
              <tr className="bg-stone-50/50 border-b border-stone-200">
                ...
                <th className="px-6 py-4 text-[10px] font-semibold text-stone-400 uppercase tracking-widest">Document Status</th>
            ...
            <tbody className="divide-y divide-stone-100">
              {currentRecords.map((record) => (
                <tr key={record.id} className="hover:bg-stone-50/30 transition-colors">
                  ...
                  <td className="px-6 py-4">
                    <span className={`px-2 py-1 rounded-md text-[9px] font-semibold uppercase tracking-wider ${
